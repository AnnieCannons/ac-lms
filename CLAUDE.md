...
- [ ] 11. Admin view — `/flashcards/admin` — instructors, staff, admin only. Chunks:
  ...
  - [ ] Chunk 4: Import activity — single admin-only page at `/flashcards/import-activity`. Linked from an "Import Activity" button on the My Decks page. Lists all shared decks owned by the admin, ordered by most recent push date (never-pushed decks at bottom). Each deck is a collapsible row (expanded by default). Expanded row shows: deck title, import count (shows "0 importers" if none), last push date. Within each expanded row: a collapsible list of importer names (collapsed by default). No course filter. The individual `/flashcards/decks/[id]/import-activity` route is removed. ChartLine button removed from DeckCard. Deck metadata (title, description, tags, course_tag) is auto-synced to all imported copies when the owner pushes updates — no user choice required.

...
- [ ] 11. Admin view — `/flashcards/admin` — instructors, staff, admin only. Chunks:
  ...
  - [x] Chunk 3: Stats page — single scrollable page (no sub-pages). Sub-chunks:
  ...
  - [ ] Chunk 5: Cloze and image occlusion card types.
    ...
    - [ ] Chunk 5b: Image occlusion cards
      ...
      - [ ] Step 3: Build image upload component for the card editor — reuse existing upload logic, store to `flashcard-images/{deckId}/`, display uploaded image below the question field
      - [ ] Step 4: Build react-konva canvas overlay on the image — click and drag to draw teal semi-transparent rectangles, click a box to select it (show resize handles + delete button)
      - [ ] Step 5: Wire up the creation form — question field (Tiptap) + image upload + canvas editor as one combined UI

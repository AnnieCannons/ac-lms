...
type Task = { id: string; label: string; order: number }
...
const DEFAULT_TASKS: Task[] = [
  { id: 'zoom-link', label: 'Zoom Link', order: 0 },
  { id: 'slack-link', label: 'Slack Link', order: 1 },
  { id: 'zoom-recordings', label: 'Zoom Recordings Link', order: 2 },
  { id: 'office-hours', label: 'Office Hours Link', order: 3 },
]
...
function LaunchSetupModal({ courseId, onClose }: { courseId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    const load = async () => {
      // Load task template from global_content
      ...
      const loadedTasks: Task[] = templateRow?.content
        ? JSON.parse(templateRow.content)
        : DEFAULT_TASKS

      setTasks(loadedTasks)

      // Load this course's saved values
      ...
      if (section) {
        ...
        try { setValues(JSON.parse(section.content ?? '{}').values ?? {}) } catch {}
      }
  ...
  const copyFromLastTime = async () => {
    ...
    if (sections && sections.length > 0) {
      // Use the last one found (most recently added in the list)
  ...
  const handleSave = async () => {
    ...
    // Save updated task template to global_content
  ...
  const addTask = () => {
    ...
    const task: Task = { id: generateId(), label: newLabel.trim(), order: tasks.length }
                ...
                {tasks.map(task => (
                  <div key={task.id} className="flex items-center gap-3 group">
                    ...
                    <button
                      ...
                      className="shrink-0 mt-5 text-border hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 text-xs"
                ...
                {addingTask ? (
                  <div className="flex items-center gap-2 mt-1">
                    <input
                      ...
                      onKeyDown={e => { if (e.key === 'Enter') addTask(); if (e.key === 'Escape') { setAddingTask(false); setNewLabel('') } }}
                      placeholder="Task label (e.g. Absence Form Link)…"
                      autoFocus
                  ...
                  <button
                    ...
                    className="text-sm text-teal-primary font-medium hover:underline self-start mt-1"
                  >
                    + Add item
                  </button>
                )}

...
export default async function InstructorGeneralInfoPage({
  ...
  return (
    <div className="min-h-screen bg-background">
      <InstructorTopNav name={profile?.name} role={profile?.role} isTa={isTa} breadcrumbs={[{ label: 'Courses', href: '/instructor/courses' }, { label: course.name, href: `/instructor/courses/${id}` }, { label: 'Course Info' }]} />
      ...
      <div className="flex">
        ...
        <div className="flex-1 min-w-0">
          <main id="main-content" tabIndex={-1} className="max-w-3xl mx-auto px-8 py-10 focus:outline-none">
            ...
            <div className="mt-6">
              <GeneralInfoEditor courseId={id} initialSections={sections ?? []} readOnly={false} />
            </div>
          </main>
        </div>
      </div>

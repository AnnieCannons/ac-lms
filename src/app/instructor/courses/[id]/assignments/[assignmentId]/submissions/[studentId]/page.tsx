...
export default async function GradingPage({
  ...
  searchParams: Promise<{ grader?: string; by?: string }>
}) {
  const { id, assignmentId, studentId } = await params
  const { grader, by } = await searchParams
  ...
  // Student mode: navigate through this student's submitted assignments
  let studentModePrev: { id: string; title: string } | null = null
  let studentModeNext: { id: string; title: string } | null = null
  let studentModePosition = 0
  let studentModeTotal = 0

  if (isStudentMode) {
    const { data: allModulesForStudent } = await admin
      .from('modules')
      .select('order, module_days(order, assignments!module_day_id(id, title, order))')
      .eq('course_id', id)

    type StudentModeDay = { order: number; assignments: { id: string; title: string; order: number }[] }
    type StudentModeModule = { order: number; module_days: StudentModeDay[] }

    const orderedForStudent = (allModulesForStudent as StudentModeModule[] ?? [])
      .sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
      .flatMap(m =>
        (m.module_days ?? [])
          .sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
    ...
    const allIds = orderedForStudent.map(a => a.id)
    if (allIds.length > 0) {
      const { data: studentSubData } = await admin
        .from('submissions')
        .select('assignment_id')
      ...
      const submittedSet = new Set(studentSubData?.map(s => s.assignment_id) ?? [])
      // Include current even if now graded (keeps position stable while grading)
      const studentAssignments = orderedForStudent.filter(a => submittedSet.has(a.id) || a.id === assignmentId)
      const currentIdx = studentAssignments.findIndex(a => a.id === assignmentId)
      studentModeTotal = studentAssignments.length
      studentModePosition = currentIdx + 1
      studentModePrev = currentIdx > 0 ? studentAssignments[currentIdx - 1] : null
      studentModeNext = currentIdx < studentAssignments.length - 1 ? studentAssignments[currentIdx + 1] : null
    }
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="flex">
        ...
        <div className="flex items-start justify-between gap-4 mb-8">
          ...
          {submission && (
            <GradeButtons
              ...
              nextUrl={
                isStudentMode
                  ? (studentModeNext ? `/instructor/courses/${id}/assignments/${studentModeNext.id}/submissions/${studentId}?by=student` : null)
                  : nextUngradedStudent
        ...
        <div className="flex items-center justify-between mt-10 pt-6 border-t border-border gap-4">
          ...
          <Link href={isStudentMode ? `/instructor/courses/${id}/submissions?tab=students` : subBase} className="text-sm text-muted-text hover:text-teal-primary transition-colors shrink-0">

...
export default function StudyPage() {
  ...
  const [cardEdits, setCardEdits] = useState<Record<string, { front: string; back: string }>>({})
  ...
  const card = queue[0]
  const front = card ? (cardEdits[card.id]?.front ?? card.front_content) : ''
  const back  = card ? (cardEdits[card.id]?.back  ?? card.back_content)  : ''
  const isTypeIn = card?.card_type === 'type_in'

  // Flip: rotate to 90°, swap content, rotate from -90° → 0°
  const doFlip = async () => {
  ...
  // Reset to front instantly (no animation) when moving to next card
  const resetToFront = () => {
  ...
  const handleEditSave = () => {
    if (!card) return
    setCardEdits(prev => ({ ...prev, [card.id]: { front: editFront, back: editBack } }))
    console.log('Update card (seed phase):', { id: card.id, front_content: editFront, back_content: editBack })
    setIsEditing(false)
  }

  const progress = sessionTotal > 0 ? completed / sessionTotal : 0

  if (!deck || sessionTotal === 0) {
    return (
    ...
    <div className="max-w-2xl mx-auto px-6 py-8 flex flex-col min-h-[calc(100vh-4rem)]">
      ...
      {isEditing ? (
        ...
        <div className="flex flex-col items-center flex-1 gap-4">
          ...
          {/* Card (single face, flip-and-swap) */}
          <div

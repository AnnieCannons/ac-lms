...
async function importCourse(filePath: string) {
  ...
  const assignments: AssignmentJSON[] = JSON.parse(raw)
  ...
  // Use the first module_title to infer the course name
  // or you can hardcode it as a CLI arg
  const courseName = path.basename(filePath, '.json')
  const courseCode = courseName.toUpperCase().replace(/\s+/g, '-')
  ...
  for (const [moduleTitle, moduleAssignments] of moduleMap) {
    ...
    if (moduleError) {
      console.error(`Failed to create module "${moduleTitle}":`, moduleError.message)
      continue
    }
    ...
    if (dayError) {
      console.error(`Failed to create day for module "${moduleTitle}":`, dayError.message)
      continue
    }
    ...
    for (let i = 0; i < moduleAssignments.length; i++) {
      ...
      if (assignmentError) {
        console.error(`    Failed to insert assignment "${a.title}":`, assignmentError.message)
      } else {
...
if (!filePath) {
  console.error('Usage: npx ts-node scripts/import-course.ts <path-to-json-file>')
  process.exit(1)
}

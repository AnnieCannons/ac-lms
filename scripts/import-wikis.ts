...
async function importWikis(filePath: string, courseNameOverride?: string) {
  ...
  for (const [moduleTitle, wikiPages] of pagesByModule) {
    ...
    // 5. Find or create "Wiki" day for this module
    ...
    if (existingDay) {
      ...
      console.log(`    ↳ Using existing Wiki day`);
    } else {
      ...
      const { data: newDay, error: dayError } = await supabase
        .from("module_days")
        .insert({ module_id: module.id, day_name: "Wiki", order: nextOrder })
        .select("id")
        .single();

      if (dayError || !newDay) {
        console.error(`    ✗ Failed to create Wiki day:`, dayError?.message);
        totalSkipped += wikiPages.length;
        continue;
      }

      dayId = newDay.id;
      console.log(`    ↳ Created Wiki day`);
    }
...
if (!filePath) {
  console.error('Usage: npx ts-node --esm scripts/import-wikis.ts <path-to-wiki-json> ["Course Name Override"]');
  process.exit(1);
}

importWikis(filePath, courseNameOverride);
